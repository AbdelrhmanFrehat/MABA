...
import { PurchaseReportItem } from '../../../shared/models/report.model';
...
@Component({
    ...
    template: `
        ...
        <div class="p-4">
            <div class="mb-4">
                <h1 class="text-2xl font-bold m-0">{{ 'admin.reports.purchases' | translate }}</h1>
                <p class="text-600 mt-2 mb-0">Review supplier invoice activity, paid amounts, and outstanding purchasing exposure over time.</p>
            </div>
            ...
            <div class="grid mb-4">
                ...
                <div class="col-12 md:col-3"><div class="summary-card"><span class="summary-label">Total Purchases</span><strong>{{ totalValue | currency:'ILS' }}</strong></div></div>
                ...
                <div class="col-12 md:col-3"><div class="summary-card"><span class="summary-label">Outstanding</span><strong>{{ totalOutstanding | currency:'ILS' }}</strong></div></div>
            </div>
            <app-data-table [data]="rows" [columns]="columns" [loading]="loading" [showAddButton]="false" [showDeleteSelected]="false" [title]="'Purchase Report'" [globalFilterFields]="['invoiceNumber', 'supplierNameEn', 'supplierNameAr']"></app-data-table>
        </div>
    `,
...
export class PurchaseReportComponent implements OnInit {
    ...
    columns: TableColumn[] = [
        ...
        { field: 'supplierNameEn', header: 'Supplier', sortable: true },
    ...
    load(): void {
        ...
        this.loading = true;
        this.reportsService.getPurchasesReport({ fromDate: value.fromDate?.toISOString(), toDate: value.toDate?.toISOString() }).subscribe({
            next: rows => { this.rows = rows ?? []; this.totalValue = this.rows.reduce((sum, row) => sum + (row.totalAmount || 0), 0); this.totalPaid = this.rows.reduce((sum, row) => sum + (row.paidAmount || 0), 0); this.totalOutstanding = this.rows.reduce((sum, row) => sum + (row.remainingAmount || 0), 0); this.loading = false; },
            error: err => { this.rows = []; this.totalValue = 0; this.totalPaid = 0; this.totalOutstanding = 0; this.loading = false; this.messageService.add({ severity: 'error', summary: 'Error', detail: err.error?.message || 'Failed to load purchase report.' }); }
        });
    }
