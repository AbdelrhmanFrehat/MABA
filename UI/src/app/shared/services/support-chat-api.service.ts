...
export class SupportChatApiService {
    ...
    /** Customer: list my conversations (newest first). */
    getMine(): Observable<SupportConversationDto[]> {
        return this.http.get<SupportConversationDto[]>(`${this.baseUrl}/mine`);
    }

    /** Customer: create a conversation with optional first message. */
    createConversation(body: CreateSupportConversationRequest): Observable<SupportConversationDto> {
        return this.http.post<SupportConversationDto>(`${this.baseUrl}`, body);
    }
    ...
    /** REST send (fallback when SignalR is down). */
    postMessage(conversationId: string, body: SendMessageRequestBody): Observable<SupportMessageDto> {
        return this.http.post<SupportMessageDto>(`${this.baseUrl}/${conversationId}/messages`, body);
    }
    ...
    closeConversation(conversationId: string): Observable<void> {
        return this.http.post<void>(`${this.baseUrl}/${conversationId}/close`, {});
    }

    reopenConversation(conversationId: string): Observable<void> {
        return this.http.post<void>(`${this.baseUrl}/${conversationId}/reopen`, {});
    }
}
