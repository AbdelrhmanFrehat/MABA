...
int stepDelay = 200;
const int MIN_STEP_DELAY_US = 60;
const int MAX_STEP_DELAY_US = 2000;

const float DEFAULT_RAPID_FEED_XY = 1200.0;
const float DEFAULT_FEED_XY = 900.0;
const float DEFAULT_FEED_Z = 300.0;
const float DEFAULT_PLUNGE_Z = 200.0;

bool X_HOME_DIR = LOW;
bool Y_HOME_DIR = LOW;
bool Z_HOME_DIR = LOW;

// Current MABA CNC hardware uses inverted physical Z motor direction relative to the
// logical app-space convention where positive Z means "up / safe travel" and negative Z
// means "down / cutting depth". Keep that mapping explicit here so firmware motion,
// jog, and streamed jobs all agree with the operator-facing CNC workflow.
const bool Z_MOTOR_DIRECTION_INVERTED = true;
...
int computeStepDelayMicros(float feed, long dx, long dy, long dz, bool rapid, bool plunge) {
  float effectiveFeed = feed;
  if (effectiveFeed <= 0) {
    effectiveFeed = rapid
      ? DEFAULT_RAPID_FEED_XY
      : (dz > 0 && dx == 0 && dy == 0 ? (plunge ? DEFAULT_PLUNGE_Z : DEFAULT_FEED_Z) : DEFAULT_FEED_XY);
  }

  if (rapid) {
    effectiveFeed = min(effectiveFeed, DEFAULT_RAPID_FEED_XY);
  } else if (dz > 0 && dx == 0 && dy == 0) {
    effectiveFeed = min(effectiveFeed, plunge ? DEFAULT_PLUNGE_Z : DEFAULT_FEED_Z);
  } else if (dz > 0 && plunge) {
    effectiveFeed = min(effectiveFeed, DEFAULT_PLUNGE_Z);
  } else if (dz > 0) {
    effectiveFeed = min(effectiveFeed, DEFAULT_FEED_Z);
  } else {
    effectiveFeed = min(effectiveFeed, DEFAULT_FEED_XY);
  }

  float dominantStepsPerMM = stepsPerMM_X;
  long dominantSteps = dx;
  if (dy > dominantSteps) {
    dominantSteps = dy;
    dominantStepsPerMM = stepsPerMM_Y;
  }
  if (dz > dominantSteps) {
    dominantSteps = dz;
    dominantStepsPerMM = stepsPerMM_Z;
  }

  if (dominantSteps <= 0 || dominantStepsPerMM <= 0 || effectiveFeed <= 0) {
    return stepDelay;
  }

  float stepsPerSecond = (effectiveFeed * dominantStepsPerMM) / 60.0;
  if (stepsPerSecond <= 0) {
    return stepDelay;
  }

  int pulseDelay = (int)(1000000.0 / (stepsPerSecond * 2.0));
  pulseDelay = constrain(pulseDelay, MIN_STEP_DELAY_US, MAX_STEP_DELAY_US);
  return pulseDelay;
}
...
void moveLinearMM(float targetX, float targetY, float targetZ, float feed, bool rapid, bool acknowledge) {
  if (!unlocked || alarm) {
  ...
  dy = abs(dy);
  dz = abs(dz);
  bool plunge = targetZ < posZ;
  int motionStepDelay = computeStepDelayMicros(feed, dx, dy, dz, rapid, plunge);
  ...
  for (long i = 0; i < maxSteps; i++) {
    ...
    delayMicroseconds(motionStepDelay);
...
void moveArc(bool clockwise, float endX, float endY, float i, float j, float feed) {
  ...
  for (int n = 1; n <= segments; n++) {
    ...
    moveLinearMM(x, y, posZ, feed, false, false);
...
void parseCommand(String s) {
  ...
  if (s.startsWith("JX")) {
    ...
    moveLinearMM(posX + distance, posY, posZ, 300, false, true);
    return;
  }

  if (s.startsWith("JY")) {
    float distance = getValue(s, 'Y', 0);
    moveLinearMM(posX, posY + distance, posZ, 300, false, true);
    return;
  }

  if (s.startsWith("JZ")) {
    float distance = getValue(s, 'Z', 0);
    moveLinearMM(posX, posY, posZ + distance, 300, false, true);
    return;
  }
