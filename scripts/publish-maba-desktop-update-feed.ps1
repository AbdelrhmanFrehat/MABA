param(
    ...
    [string]$PackagePath = "C:\Users\PC\Desktop\maba\artifacts\desktop-updates\stable\MabaControlCenter-0.1.22-win-x64.zip",
...
$multipart.Add($fileContent, 'file', [System.IO.Path]::GetFileName($PackagePath))
$multipart.Add((New-Object System.Net.Http.StringContent($Version)), 'version')
$multipart.Add((New-Object System.Net.Http.StringContent("Desktop app release $Version")), 'notes')
$multipart.Add((New-Object System.Net.Http.StringContent($Channel)), 'channel')

$publishHttp = $client.PostAsync("$ApiBaseUrl/api/v1/desktop-updates/publish", $multipart).GetAwaiter().GetResult()
...
if (-not $publishHttp.IsSuccessStatusCode) {
    throw "Publish update failed: $($publishHttp.StatusCode) $publishBody"
}
